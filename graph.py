...
for (local_index, extended_index) in zip(["lhn1", "hpi", "hdi"], ["lhn1_e", "hpi_e", "hdi_e"]):
	fig, ax = plt.subplots()
	ax.plot(list(plot_vals["cn"].keys()), [list_avg(l) for l in plot_vals["cn"].values()], linestyle = '--', marker = 'o', color = (0.55, 0.55, 0.55), label = "CN")
	ax.plot(list(plot_vals["lp"].keys()), [list_avg(l) for l in plot_vals["lp"].values()], linestyle = '--', marker = 'x', color = (0.45, 0.45, 0.45), label = "LP")
	ax.plot(list(plot_vals[local_index].keys()), [list_avg(l) for l in plot_vals[local_index].values()], linestyle = '-', marker = 'o', color = (0, 0.6, 0), label = local_index.upper())
	ax.plot(list(plot_vals[extended_index].keys()), [list_avg(l) for l in plot_vals[extended_index].values()], linestyle = '-', marker = 'x', color = (0, 0.8, 0), label = extended_index.upper())
	ax.set_xlabel('Average degree')
	ax.set_ylabel('AUC')
	ax.set_title("{} and {}".format(local_index.upper(), extended_index.upper()))
	ax.legend(["CN", "LP", local_index.upper(), extended_index.upper()])
	plt.savefig("test_" + local_index)
	plt.clf()
fig, ax = plt.subplots()
ax.plot(list(plot_vals["cn"].keys()), [list_avg(l) for l in plot_vals["cn"].values()], linestyle = '--', marker = 'o', color = (0.55, 0.55, 0.55), label = "CN")
ax.plot(list(plot_vals["lp"].keys()), [list_avg(l) for l in plot_vals["lp"].values()], linestyle = '--', marker = 'x', color = (0.45, 0.45, 0.45), label = "LP")
ax.plot(list(plot_vals["ra"].keys()), [list_avg(l) for l in plot_vals["ra"].values()], linestyle = '-', marker = 'o', color = (0, 0.5, 0), label = "RA")
ax.plot(list(plot_vals["ra_e"].keys()), [list_avg(l) for l in plot_vals["ra_e"].values()], linestyle = '-', marker = 'x', color = (0, 0.75, 0), label = "RA_E")
ax.plot(list(plot_vals["ra_e2"].keys()), [list_avg(l) for l in plot_vals["ra_e2"].values()], linestyle = '-', marker = '^', color = (0, 0.95, 0), label = "RA_E2")
ax.legend(["cn", "lp", local_index, extended_index])
ax.set_xlabel('Average degree')
ax.set_ylabel('AUC')
ax.set_title("{}, {}, and {}".format("RA", "RA_E", "RA_E2"))
ax.legend(["CN", "LP", "RA", "RA_E", "RA_E2"])
plt.savefig("test_ra")
exit()
